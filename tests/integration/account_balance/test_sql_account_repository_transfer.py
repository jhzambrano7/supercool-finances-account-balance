"""Integration tests for the transfer-specific `AccountRepository` behaviour

(T6, T7, T9) against a real PostgreSQL: `get_for_update`, `update`, and a
`SYSTEM` account's balance being computed from its entries rather than read
from `balance_amount`.
"""
...
async def _post_funding_entry(
    ...
    *,
    ...
    """Writes a bare `transfers`/`entries` row pair directly -- this test

    only needs a `SYSTEM` account to have entries against it, not a fully
    posted, balanced `Transfer` (that is `TransferMoneyUseCase`'s own
    concern, exercised by `test_transfer_route.py`).
    """
    transfer_id = uuid4()
    async with session_factory() as session:
        await session.execute(
            insert(TransferRow).values(
...
async def test_get_computes_system_balance_from_entries_not_the_stored_column(
    ...
    funding = await repository.get(account_id)
...
async def test_get_for_update_never_returns_a_system_account(
    ...
    """T6: a `SYSTEM` account is never locked -- `get_for_update` must not

    hand one back even though the row exists.
    """
    repository = SqlAccountRepository(session_factory)
...
async def test_update_persists_a_user_accounts_new_balance_and_version(
    ...
    """`update()` deliberately does not commit on its own (T3: the enclosing

    `TransferUnitOfWork` commits once, atomically) -- so this test goes
    through `SqlTransferUnitOfWork`, the only way `update()` is ever really
    used, rather than calling it against a bare, per-call session that would
    roll back before anything could be observed.
    """
    account_repository = SqlAccountRepository(session_factory)
    account = _open_user_account(owner_id=OwnerId(uuid4()))
    await account_repository.add(account)

    unit_of_work = SqlTransferUnitOfWork(session_factory)
    ...
    reloaded = await account_repository.get(account.account_id)
