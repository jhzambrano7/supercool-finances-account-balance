...
`sql_collections_repository.py`/`negative_balances.py`). Age is surfaced as descriptive buckets
(under a day / 1–7 days / 7–30 days / over 30 days), not as a write-off decision — §12 is still
open. This is a collections screen, not the alerting/exporter this section otherwise descopes: it
answers "who is negative right now", on demand, from an operator-authorized caller, the same
authorization mechanism §7.1 already established for reversal — it does not export a time series or
