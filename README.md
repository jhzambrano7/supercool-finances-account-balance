...
| Collections view: `GetCollectionsReport`, `GET /collections`, `SqlCollectionsRepository` | **Built** — PRD §11.3's risk signals as a real, operator-only endpoint (not the descoped metrics exporter below): every negative `USER` balance, the exposure it represents per currency, and the start of its *current* negative episode (a SQL window-function query over `entries`, correct across a recovered-then-negative-again account — see `queries/negative_balances.py`). A drifted balance (negative with no explaining entry history, PRD §11.1) is surfaced, not dropped: the join is `LEFT`, and such an account reports `negative_since: null` |
| Demo web console (`web/`) | **Built** — a React ops UI exercising every capability above, including a Collections screen; see `web/README.md`. Deliberately untested: it is a presentation facility, not a deliverable |
| Observability | **Descoped** — designed, deliberately not built; see [below](#observability-designed-and-descoped). The one exception is the collections view above: an operator-facing read endpoint over one of PRD §11.3's signals, not a metrics/alerting exporter, so it is not part of that descope |
...
| "Negative since" is a SQL window-function query over `entries` | Recompute the running balance in the application layer | The whole ledger for every negative account would have to cross the network first, for a computation the database can already do in one round trip; a naive "oldest debit" or "first crossing" also silently reports the *first* negative episode forever, not the current one, once an account recovers and goes negative again |
| Collections reuses `AuthorizationGateway`, not a new authorization concept | A separate "operator" role check | A collections list is, by construction, an enumeration of other customers' debts — the same reasoning that makes reversal operator-only (§7.1) applies with equal force; a second mechanism would be two places that decide who counts as an operator |
| Collections joins to the episode-start query with `LEFT JOIN`, and surfaces `negative_since: null` | `INNER JOIN`, since "the account is negative right now" seemed to guarantee a crossing row exists | That guarantee is exactly the assumption §11.1's balance-drift signal exists to distrust; an `INNER JOIN` would make a drifted account (negative in `accounts`, no entry history that explains it) vanish from the report and its exposure total with no row, no null, no error — under-reporting the one number this screen exists to get right |
